...
public class DomainInfo implements /*FavIconRequestor,*/ Comparable<DomainInfo> {
    ...
    public synchronized ImageIcon getFavIcon() {
        ...
        if (!hosterIconRequested) {
            ...
            // load it
            ia = null;//FavIcons.getFavIcon(getTld(), this, true);
            if (ia != null) {
                ia = setFavIcon(ia);
                return ia;
            }
    ...
    public synchronized ImageIcon setFavIcon(ImageIcon icon) {
        if (icon == null) {
            ...
            if (icon == null) {
                icon = null;//FavIcons.getFavIcon(getTld(), this, true);
            }
        } else {
            icon = null;//new ImageIcon(IconIO.getScaledInstance(icon.getImage(), WIDTH, HEIGHT, Interpolation.BICUBIC, true));
        }
...
//        }
        ...
        return null;//new ImageIcon(IconIO.getScaledInstance((BufferedImage) ret.getImage(), size, size));

        }
        if (!hosterIconRequested) getFavIcon();
        ret = null;//FavIcons.getFavIcon(getTld(), null, true);
