...
public class JSonStorage {
        ...
        synchronized (JSonStorage.LOCK) {
            ...
            try {
                ...
                if (tmpfile.exists() && tmpfile.length() > 0) {
                    ...
                    try {
                        ...
                        if (plain) {
                            ...
                            throw new RuntimeException("Not supported");//ret = JSonStorage.restoreFromString(stri = Crypto.decrypt(str, key), type, def);
                        }
                ...
                if (plain) {
                    ...
                    throw new RuntimeException("Not supported");//return JSonStorage.restoreFromString(stri = Crypto.decrypt(str, key), type, def);
                }
    ...
    public static <E> E restoreFromString(byte[] data, boolean plain, byte[] key, TypeRef<E> type, E def) {
        ...
        try {
            ...
            if (!plain) {
                throw new RuntimeException("Not supported");//string = Crypto.decrypt(data, key);
            } else {
        ...
        synchronized (JSonStorage.LOCK) {
            try {
                ...
                if (plain) {
                    ...
                    /* encrypted */
                    throw new RuntimeException("Not supported");//IO.writeToFile(tmp, Crypto.encrypt(json, key));
                }
                if (file.exists()) {
