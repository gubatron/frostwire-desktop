	...
	implements TRTrackerAnnouncerHelper
{
  ...
  TRTrackerBTAnnouncerImpl2(
   	String						_sha1,
   	String[]		_announce_urls)
  	
  	throws TRTrackerAnnouncerException
  {
	sha1			= _sha1;
	final URL[] urls   = new URL[_announce_urls.length];
    
    for (int i=0;i<_announce_urls.length;i++){
    
        try {
            urls[i] = StringInterner.internURL(new URL(_announce_urls[i]));
        } catch (MalformedURLException e) {
            e.printStackTrace();
        }
    }
	TOTorrentAnnounceURLSet _announce_urls_set = new TOTorrentAnnounceURLSet() {
	...
	announce_urls	= new TOTorrentAnnounceURLSet[] {_announce_urls_set};
  	peer_networks	= null;
  	manual_control	= true;
  	helper			= getHelper();
	...
	constructTrackerUrlLists( true );
		...
		data_peer_id = PeerUtils.createPeerID();
	...
	try {
		...
		torrent_hash = new HashWrapper(sha1.getBytes());
		...
		Logger.log(new LogEvent(sha1, LOGID, "URL encode fails", e));
		...
		new TimerEventPerformer()
		{
				...
				TimerEvent	this_event )
			{
				...
				try{
					...
					if (Logger.isEnabled())
						Logger.log(new LogEvent(sha1, LOGID,
	...
	public void
	cloneFrom(
		TRTrackerBTAnnouncerImpl2	other )
	{
		helper					= other.helper;
	...
	protected TRTrackerAnnouncerResponseImpl startSupport() {
		if (Logger.isEnabled())
			Logger.log(new LogEvent(sha1, LOGID, "Tracker Announcer is sending "
					+ "a start Request"));
	  		...
	  		for (int i=0;i<peers.length;i++){
	  			...
	  			if ( peer_networks == null ){
		  			...
		  			if (!added && Logger.isEnabled())
							Logger.log(new LogEvent(sha1, LOGID, LogEvent.LT_WARNING,
 		...
 		try{
	    	TorrentUtils.setTLSTorrentHash( torrent_hash );
	 		...
	 		for (int i=0;i<2;i++){	
					...
					if (Logger.isEnabled()){
			  		...
			  		if ( udpAnnounceURL != null ){
						...
						if ((failure_reason != null || message.size() == 0) && udp_probe){
							...
							if (Logger.isEnabled()){
								Logger.log(new LogEvent(sha1, LOGID, LogEvent.LT_INFORMATION, "redirection of http announce [" + tracker_url[0] + "] to udp failed, will retry in " + autoUDPprobeEvery + " announces"));
							}	
							...
							if (Logger.isEnabled()){
								Logger.log(new LogEvent(sha1, LOGID, LogEvent.LT_INFORMATION, "redirection of http announce [" + tracker_url[0] + "] to udp successful"));
							}
				...
				if (Logger.isEnabled())
					Logger.log(new LogEvent(sha1, LOGID, errorLevel ? LogEvent.LT_ERROR : LogEvent.LT_WARNING,
 	...
 	{ 		
 		...
 		try{
 			...
 			try{
 				...
 				while ( content_length <= 0 || num_read < content_length ){
 					...
 					try{
 						...
 						if (Logger.isEnabled()) {
							...
							Logger.log(new LogEvent(sha1, LOGID, LogEvent.LT_ERROR,
 				...
 				if (Logger.isEnabled())
					Logger.log(new LogEvent(sha1, LOGID, "Tracker Announcer ["
	 		...
	 		try{
	 			...
	 			try{
	 				...
	 				try{
	 					...
	 					if ( 	b_warning_message != null && 
							...
							if ( !warning_message.equals( last_tracker_message )){
								...
								if ( log_it ){
		 							Logger.logTextResource(new LogAlert(sha1, 
					...
					try {
						...
						if (Logger.isEnabled()) {
							Logger.log(new LogEvent(sha1, LOGID, LogEvent.LT_INFORMATION,
			    	...
			    	if ( az_compact == 2 ){
				  	 ...
				  	 if (Logger.isEnabled()) {
				    	...
				    	for (int i=0;i<meta_peers.length;i+=entry_size){
				    		...
				    		if (Logger.isEnabled())
								Logger.log(new LogEvent(sha1, LOGID, "COMPACT PEER: " + peer.getString()));
						...
						for (int i=0; i<v6peers.length; i+=entry_size ){
							...
							if (tcp_port < 0 || tcp_port > 65535){
								...
								if (Logger.isEnabled()){
									...
									Logger.log(new LogEvent(sha1, LOGID, LogEvent.LT_ERROR, "Invalid compactv6 peer port given: " + ip + ": " + tcp_port));
								}
							...
							if (Logger.isEnabled()){
								...
								Logger.log(new LogEvent(sha1, LOGID, "COMPACTv6 PEER: " + peer.getString()));
							}
		...
		if ( result.getResponseType() == DownloadAnnounceResult.RT_ERROR ){
			...
			for (int i=0;i<ext_peers.length;i++){
				...
				if ( !ps_enabled && ps.equals( PEPeerSource.PS_BT_TRACKER )){
					...
					if (Logger.isEnabled())
						Logger.log(new LogEvent(sha1, LOGID, "EXTERNAL PEER: " + p.getString())); 
				}
			}
	    ...
	    try {
            ...
            t.addListener(new TRTrackerAnnouncerListener() {
                ...
                public void receivedTrackerResponse(TRTrackerAnnouncerResponse response) {
                    System.out.println("TRTrackerAnnouncerResponse");
                }
            });
            t.setAnnounceDataProvider(new TRTrackerAnnouncerDataProvider() {
