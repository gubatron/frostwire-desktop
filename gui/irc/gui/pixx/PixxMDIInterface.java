...
class MDILayout implements LayoutManager {
    ...
    public Dimension preferredLayoutSize(Container parent) {
        ...
        if (visible != null)
            return visible.getPreferredSize();
        return new Dimension(0, 0);
    }
...
public class PixxMDIInterface extends IRCInterface implements PixxTaskBarListener, PixxMenuBarListener, ActionListener, MouseWheelPanelListener,
        ...
        GUIMediator.safeInvokeLater(new Runnable() {
            public void run() {
                ...
                if (!bring.booleanValue())
                    _pixxConfiguration.getIRCConfiguration().getAudioConfiguration().onQuery();
            }
        });
    ...
    private void chanListCreated(final ChanList list, final Boolean bring) {
        GUIMediator.safeInvokeLater(new Runnable() {
            public void run() {
                ...
                cl.addBaseAWTSourceListener(PixxMDIInterface.this);
                _lists.put(list, cl);
            }
        });
    }
    ...
    public void sourceCreated(Source source, Boolean bring) {
        ...
        if (source instanceof Channel)
            channelCreated((Channel) source, bring);
        if (source instanceof Query)
            queryCreated((Query) source, bring);
        if (source instanceof Status) {
            if (_pixxConfiguration.getB("showstatus"))
                statusCreated((Status) source, bring);
        }
        if (source instanceof ChanList)
            chanListCreated((ChanList) source, bring);
        if (source instanceof DCCChat)
            DCCChatCreated((DCCChat) source, bring);
        if (source instanceof DCCFile)
            DCCFileCreated((DCCFile) source, bring);
    }
    ...
    public GUISource getGUISource(Source source) {
        if (source instanceof DefaultSource)
            return _awtDefaultSource;
        if (source instanceof Channel)
            return (GUISource) _channels.get(source);
        if (source instanceof Query)
            return (GUISource) _queries.get(source);
        if (source instanceof Status)
            return (GUISource) _status.get(source);
        if (source instanceof ChanList)
            return (GUISource) _lists.get(source);
        if (source instanceof DCCChat)
            return (GUISource) _dccChats.get(source);
        if (source instanceof DCCFile)
            return (GUISource) _dccFiles.get(source);
        return null;
    }
    ...
    public void dock(BaseAWTSource source) {
        ...
        source.requestFocus();
        if (_pixxConfiguration.getB("hideundockedsources"))
            _task.show(source);
    }
    ...
    public void AWTSourceAdded(PixxTaskBar bar, BaseAWTSource asource) {
        ...
        _panel.validate();
        test();
        if (_pixxConfiguration.getDockingPolicy(asource.getSource().getType().toLowerCase(java.util.Locale.ENGLISH),
                asource.getSource().getName().toLowerCase(java.util.Locale.ENGLISH)))
            undock(asource);
    }
    ...
    public void helpClicked(PixxMenuBar bar) {
        ...
        if (_pixxConfiguration.getS("helppage") != null)
            cfg.getURLHandler().openURL(_pixxConfiguration.getS("helppage"));
    }
