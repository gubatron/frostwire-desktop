...
public final class GUIMediator {
	...
	public static enum Tabs {
		...
		private Tabs(String nameWithAmpers, BooleanSetting visibleSetting) {
			...
			navAction = new NavigationAction(nameWithAmpers, I18n.tr(
					"Display the {0} Screen", name));
		...
		void setName(String newName) {
			...
			navAction.putValue(Action.LONG_DESCRIPTION, I18n.tr(
					"Display the {0} Screen", name));
			propertyChangeSupport.firePropertyChange("name", oldName, name);
			showTabAction.putValue(Action.NAME, newName);
			showTabAction.putValue(Action.LONG_DESCRIPTION, I18n.tr(
					"Toggle the {0} Screen", name));
		}
		...
		public boolean isViewEnabled() {
			if (visibleSetting == null) {
				throw new IllegalStateException("Should not be called on "
						+ getName() + " which is a non-optional tab");
			}
		...
		private class ShowTabAction extends AbstractAction {
			...
			private ShowTabAction(GUIMediator.Tabs tab) {
				super(tab.getName());
				putValue(LONG_DESCRIPTION, I18n.tr("Show {0} Window", tab
						.getName()));
				this.tab = tab;
				tab.addPropertyChangeListener(new PropertyChangeListener() {
					public void propertyChange(PropertyChangeEvent evt) {
						if ("name".equals(evt.getPropertyName())) {
							...
							putValue(LONG_DESCRIPTION, I18n.tr(
									"Show {0} Window", evt.getNewValue()));
						}
					}
				});
			}
			...
			public void actionPerformed(ActionEvent ae) {
				...
				GUIMediator.instance().getMainFrame().getMenuMediator()
						.refreshShowHideSubMenu();
			}
		}
	...
	private static final void showDialogsForFirstVisibility() {
		...
		JDialog dialog = JavaVersionNotice
				.getUpgradeRecommendedDialog(VersionUtils.getJavaVersion());
		if (dialog != null) {
	 ...
	 */
	public static final ResourceBundle getXMLResourceBundle(
	 ...
	 */
	public static final String showInputMessage(final String message,
			String initialValue) {
		return MessageService.instance()
				.showInputMessage(message, initialValue);
	}
	 ...
	 */
	public static final DialogOption showYesNoMessage(final String message,
			final DialogOption defaultOption) {
		return MessageService.instance().showYesNoMessage(message,
				defaultOption);
	}
	 ...
	 */
	public static final DialogOption showYesNoMessage(final String message,
			final IntSetting defaultValue, final DialogOption defaultOption) {
		return MessageService.instance().showYesNoMessage(message,
	...
	public static final DialogOption showYesNoTitledMessage(
			...
			final DialogOption defaultOption) {
		return MessageService.instance().showYesNoMessage(message, title,
				defaultOption);
	}
	...
	public static final int showConfirmListMessage(final String message,
			final Object[] listModel, int messageType,
			final ListCellRenderer listRenderer) {
		return MessageService.instance().showConfirmListMessage(message,
				listModel, messageType, listRenderer);
	}
	...
	public static final DialogOption showYesNoOtherMessage(
			...
			String otherOptions) {
		return MessageService.instance().showYesNoOtherMessage(message,
	 ...
	 */
	public static final void showDisposableMessage(final String messageKey,
			final String message, final Switch ignore, int msgType) {
		MessageService.instance().showDisposableMessage(messageKey, message,
				ignore, msgType);
	}
	...
	public static final void launchFile(File file) {
		try {
			...
			showError(I18n
					.tr("FrostWire will not launch the specified file for security reasons."));
