...
final class LibraryTree extends JTree implements MouseObserver {
    ...
    private final SavedFilesDirectoryHolder torrentsfdh = new SavedFilesDirectoryHolder(SharingSettings.TORRENT_DATA_DIR_SETTING,
    ...
    private void makeButtonRow() {
        if (hasExploreAction()) {
            BUTTON_ROW = new ButtonRow(new Action[] { refreshAction, exploreAction, configureSharingAction }, ButtonRow.X_AXIS, ButtonRow.NO_GLUE);
        } else {
            BUTTON_ROW = new ButtonRow(new Action[] { refreshAction, configureSharingAction }, ButtonRow.X_AXIS, ButtonRow.NO_GLUE);
        }
    }
