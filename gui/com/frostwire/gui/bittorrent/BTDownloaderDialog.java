 ...
 */
public class BTDownloaderDialog extends JDialog {
     ...
     * @param sle
     */
    BTDownloaderDialog(BTDownloaderFactory factory, SaveLocationException sle) {
        ...
        if(defaultAction != DEFAULT_ASK) {
            if(defaultAction == DEFAULT_APPEND)
                new AppendAction().actionPerformed(new ActionEvent(BTDownloaderDialog.this,0,null));
            else if(defaultAction == DEFAULT_SAVE_AS)
                new SaveAsAction().actionPerformed(new ActionEvent(BTDownloaderDialog.this,0,null));
            return;
        }
	...
	private void setContentFromException(SaveLocationException sle) {
		...
		if (sle.getErrorCode() == SaveLocationException.FILE_ALREADY_DOWNLOADING) {
			dispose();
			BTDownloaderUtils.showIsAlreadyDownloadingWarning(factory);
			return;
		}
...
//		FileDesc desc = BTDownloaderUtils.getFromLibrary(factory.getURN());
//		if (desc != null) {
...
//		}
	...
	public static BTDownloader handle(BTDownloaderFactory factory,
		...
		BTDownloaderDialog dlg = new BTDownloaderDialog(factory, sle);
		if(defaultAction == 0) {
	...
	void createUniqueFilenameDownloader() {
		...
		//Set<?> downloads = BTDownloadMediator.instance().getFileNames();
...
//		}
	...
	private class SaveAsAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			File file = BTDownloaderUtils.showFileChooser(factory, 
					BTDownloaderDialog.this);
			if (file != null) {
				try {
