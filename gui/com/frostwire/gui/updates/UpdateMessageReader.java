...
public final class UpdateMessageReader implements ContentHandler {
	...
	public void addOverlay(UpdateMessage msg) {
		if (msg != null && msg.getMessageType().equals("overlay")) {
			...
			System.out
					.println("UpdateManager.addOverlay() - The message given wasn't good.");
		}
	}
	...
	private boolean isMessageEligibleForMyOs(UpdateMessage msg) {
		...
		boolean im_mac_msg_for_me = msg.getOs().equals("mac")
				&& OSUtils.isMacOSX();

		boolean im_windows_msg_for_me = msg.getOs().equals("windows")
				&& (OSUtils.isWindows() || OSUtils.isWindowsXP()
						|| OSUtils.isWindowsNT() || OSUtils.isWindows98()
						|| OSUtils.isWindows95() || OSUtils.isWindowsMe() || OSUtils
						.isWindowsVista());

		boolean im_linux_msg_for_me = msg.getOs().equals("linux")
				&& OSUtils.isLinux();

		return im_mac_msg_for_me || im_windows_msg_for_me
				|| im_linux_msg_for_me;
	}
	...
	public void readUpdateFile() {
		...
		try {
			...
			System.out
					.println("UpdateMessageReadre.readUpdateFile() Socket Timeout Exeception "
							+ e3.toString());
		} catch (IOException e) {
			System.out
					.println("UpdateMessageReader.readUpdateFile() IO exception "
							+ e.toString());
		} catch (SAXException e2) {
			System.out
					.println("UpdateMessageReader.readUpdateFile() SAX exception "
							+ e2.toString());
		}
	}
	...
	public void setUpdateMessage(UpdateMessage msg) {
		if (_updateMessage == null && msg != null
				&& msg.getMessageType().equals("update")) {
			_updateMessage = msg;
		}
	}
	...
	public void startElement(String uri, String localName, String qName,
		...
		if (localName.equalsIgnoreCase("update")) {
			...
			if (atts.getValue("valueInstallerReady") != null) {
				_bufferMessage.setMessageInstallerReady(atts
						.getValue("valueInstallerReady"));
				// System.out.println("UpdateMessageReader.startElement overlay md5="
				// + atts.getValue("md5"));
			}

			if (_bufferMessage.getMessageType()
			...
			if (_bufferMessage.getMessageType().equalsIgnoreCase("overlay")) {
				...
				if (atts.getValue("intro") != null
						&& (atts.getValue("intro").equals("1")
								|| atts.getValue("intro").equalsIgnoreCase(
										"true") || atts.getValue("intro")
